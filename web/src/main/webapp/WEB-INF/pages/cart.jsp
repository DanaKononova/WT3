        ...
        <frm:form method="put" modelAttribute="cartItemsQuantities" action="${pageContext.servletContext.contextPath}/cart">
          <table class="table table-hover table-bordered text-center">
            ...
            <c:forEach var="i" begin="0" end="${cart.items.size() - 1}">
              <tr id="row${cart.items[i].jewelry.id}">
                <td class="align-middle">
                  <a href="<c:url value="/productDetails/${cart.items[i].jewelry.id}"/>">${cart.items[i].jewelry.brand}</a>
                </td>
                <td class="align-middle">
                  <a href="<c:url value="/productDetails/${cart.items[i].jewelry.id}"/>">${cart.items[i].jewelry.model}</a>
                </td>
                <td class="align-middle">
                  <ul>
                    <c:forEach var="color" items="${cart.items[i].jewelry.colors}">
                      <li>${color.code}</li>
                    </c:forEach>
                  </ul>
                </td>
                <td class="align-middle">${cart.items[i].jewelry.displaySizeInches}"</td>
                <td class="align-middle">$ ${cart.items[i].jewelry.price}</td>
                <td class="align-middle">
                  <frm:input path="items[${i}].quantity"/>
                  <frm:hidden path="items[${i}].jewelryId"/>
                ...
                <td class="align-middle">
                  <button form="deleteCartItem" class="btn btn-outline-dark border-dark"
                          formaction="<c:url value="/cart?jewelryId=${cart.items[i].jewelry.id}"/>">Delete</button>
                </td>
              </tr>
            </c:forEach>
          </table>
